...
function Comprobante({ datos, onNuevaReserva }) {
  const exitosos = datos.resultados.filter((r) => r.ok);
  const fallidos = datos.resultados.filter((r) => !r.ok);
  const horarioTexto = `${formatoHora(datos.horaInicio)} a ${formatoHora(datos.horaFin)}`;

  const lineasFechas = datos.repetirSemanal
    ? exitosos.map((r) => `• ${r.fechaLegible}`).join("\n")
    : exitosos[0]?.fechaLegible ?? "";

  const lineasFallidas = fallidos.length
    ? `\n\nNo se pudieron reservar (ya estaban ocupadas):\n` +
      fallidos.map((r) => `• ${r.fechaLegible}`).join("\n")
    : "";

  const mensaje =
    `Hola! Envío el comprobante de reserva de aula.\n\n` +
    `Instituto: ${NOMBRE_INSTITUTO}\n` +
    `Código: ${datos.codigoBase}\n` +
    `Nombre: ${datos.nombre}\n` +
    `Rol: ${datos.rol === "profesor" ? "Profesor" : "Profesional"}\n` +
    `Aula: ${datos.aulaNombre}\n` +
    `Horario: ${horarioTexto}\n` +
    (datos.repetirSemanal ? `Fechas reservadas:\n${lineasFechas}` : `Fecha: ${lineasFechas}`) +
    lineasFallidas;
    ...
    <div className="max-w-md mx-auto">
      <div className="rounded-2xl border border-line bg-surface p-6 sm:p-8 text-center shadow-sm">
        ...
        <div className="text-left space-y-3 border-t border-b border-line py-5 mb-5">
          ...
          <FilaComprobante etiqueta="Horario" valor={horarioTexto} />

          {!datos.repetirSemanal ? (
            <FilaComprobante etiqueta="Fecha" valor={exitosos[0]?.fechaLegible ?? "—"} />
          ) : (
            <div>
              <p className="text-sm text-ink-soft mb-1.5">Fechas reservadas</p>
              <ul className="space-y-1">
                {exitosos.map((r) => (
                  <li key={r.fechaISO} className="text-sm font-medium text-ink flex items-center gap-1.5">
        ...
        {fallidos.length > 0 && (
          <div className="text-left rounded-xl bg-needle/10 border border-needle/30 px-4 py-3 mb-5">
            <p className="text-sm font-medium text-needle mb-1">
              {fallidos.length === 1 ? "Una fecha no se pudo reservar" : `${fallidos.length} fechas no se pudieron reservar`}
            </p>
            <ul className="space-y-0.5">
              {fallidos.map((r) => (
                <li key={r.fechaISO} className="text-xs text-needle">
                  {r.fechaLegible} — ese horario ya estaba ocupado
                </li>
              ))}
            </ul>
          </div>
        ...
        <p className="font-mono text-lg tracking-wide text-brass-dark mb-6">{datos.codigoBase}</p>
...
export default function App() {
  ...
  const slotsDisponibles = useMemo(() => {
    ...
    if (rol === "profesor") {
      const lista = [];
      for (let h = HORA_APERTURA; h + 1 <= HORA_CIERRE; h += 1) {
    ...
    const lista = [];
    for (let h = HORA_APERTURA; h + 3 <= HORA_CIERRE; h += 1) {
  ...
  async function confirmarReserva() {
    ...
    // Chequeo de colisión de último momento contra los datos en vivo (solo la fecha visible)
    ...
    try {
      ...
      const fechasAProcesar = repetirSemanal
        ? [0, 7, 14, 21].map((dias) => sumarDias(fecha, dias))
        : [fecha];
      const codigoBase = `#BE-${Math.floor(1000 + Math.random() * 9000)}`;

      const resultados = [];

      for (let i = 0; i < fechasAProcesar.length; i++) {
        const f = fechasAProcesar[i];
        const fISO = formatoISO(f);
        const fLegible = formatoLegible(f);

        // Volvemos a chequear en vivo contra Firestore para cada fecha,
        // porque de las semanas futuras no tenemos un listener abierto.
        const q = query(
          collection(db, "reservas"),
          where("aula", "==", aulaId),
          where("fecha", "==", fISO)
        ...
        const ocupado = snap.docs.some((docu) => {
          const r = docu.data();
          return r.horaInicio < slotElegido.fin && r.horaFin > slotElegido.inicio;
        });

        if (ocupado) {
          resultados.push({ fechaISO: fISO, fechaLegible: fLegible, ok: false });
          continue;
        }

        const codigo = fechasAProcesar.length > 1 ? `${codigoBase}-S${i + 1}` : codigoBase;
        ...
        resultados.push({ fechaISO: fISO, fechaLegible: fLegible, ok: true, codigo });
      }
      ...
      if (!huboExito) {
        setError(
          repetirSemanal
            ? "No se pudo reservar ninguna de las 4 fechas: ese horario ya está ocupado en todas."
            ...
            <section className="rounded-2xl border border-line bg-surface p-4 sm:p-5">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  className="mt-1 h-4 w-4 rounded border-line accent-brass"
                />
                <span>
                  <span className="block text-sm font-medium text-ink">
                    Repetir este horario las próximas 4 semanas
                  </span>
                  <span className="block text-xs text-ink-soft mt-0.5">
                    Se reserva el mismo día de la semana y el mismo horario, una vez por semana.
                    Si alguna de esas fechas ya está ocupada, se omite y te avisamos cuál.
                  </span>
                </span>
              </label>
            </section>
