...
const NUMERO_WHATSAPP = "549112804691757";

const HORA_APERTURA = 8; // 08:00
const HORA_CIERRE = 22; // 22:00 (última reserva de profesional termina a esta hora)

const AULAS = [
  { id: "norte", nombre: "Norte", capacidad: "3 personas", area: "norte" },
  { id: "este", nombre: "Este", capacidad: "4 personas", area: "este" },
  { id: "sur", nombre: "Sur", capacidad: "6 personas", area: "sur" },
  { id: "oeste", nombre: "Oeste", capacidad: "4 personas", area: "oeste" },
  { id: "sum", nombre: "S.U.M.", capacidad: "8 o más personas", area: "sum" },
];
...
// ============================================================
// COMPONENTE: Selector de aula en forma de brújula
// Las aulas se llaman como los puntos cardinales, así que el
// layout codifica información real, no es solo decoración.
// ============================================================
function SelectorAulas({ aulaId, onSeleccionar }) {
  return (
    <div
      ...
      style={{
        gridTemplateAreas: `". norte ." "oeste sum este" ". sur ."`,
      ...
      {AULAS.map((aula) => {
        ...
        return (
          <button
            ...
            style={{ gridArea: aula.area }}
            className={[
              "flex flex-col items-center justify-center gap-0.5 rounded-xl border px-2 py-3 text-center transition-all",
              activa
                ? "border-brass bg-ink text-parchment shadow-md scale-[1.03]"
                : "border-line bg-surface text-ink hover:border-brass",
            ].join(" ")}
          >
            <span className="font-display font-semibold text-sm leading-tight">{aula.nombre}</span>
            <span className={`text-[11px] flex items-center gap-1 ${activa ? "text-parchment/80" : "text-ink-soft"}`}>
              <Users size={11} /> {aula.capacidad}
            </span>
          </button>
        );
      })}
